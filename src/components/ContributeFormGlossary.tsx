...
export interface ContributeFormGlossaryProps {
  bg: string;
  color: string;
  glossaryId: string;
}

export const ContributeFormGlossary: React.FC<ContributeFormGlossaryProps> = ({
  bg,
  color,
  glossaryId,
}) => {
  ...
  const onSubmit = (data: any) => {
    ...
    add(dataObj)
      .then(() => {
        ...
        // });
        router.push(
          `/technology-glossary/${router.query.slug}/contribution/success`,
        );
      })
      .catch((errors) => {
        toast({
        ...
        <Box>
          <FormControl id="sourceLink">
            ...
            <Input
              ...
              type="url"
              name="sourceLink"
              placeholder="e.g. https://tech-experts.com/term"
              ref={register()}
            />
          </FormControl>
        </Box>
