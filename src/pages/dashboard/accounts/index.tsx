...
export const DashboardAccounts = (props: any) => {
  ...
  const { data: accountsData } = useCollection<any>('accounts', {
    orderBy: ['createdAt', 'desc'],
  });

  const { deleteDocument } = useDocument<any>(
    deleteId ? `accounts/${deleteId}` : null,
  );
  ...
  const handleDelete = async () => {
    ...
    toast({
      ...
      description: "You've successfully deleted an account!",
      status: 'success',
      duration: 5000,
      isClosable: true,
    });
  ...
  return (
    <>
      {accountsData && (
        <LayoutDashboard title="Accounts" bg={props.bg} color={props.color}>
                  ...
                  >
                    <Box>
                      ...
                      <TableActions
                        ...
                        newLink={'/dashboard/accounts/new'}
                      ...
                      <Table variant="simple" my={4}>
                        ...
                        <Tbody>
                          {accountsData
                            ?.filter(
                              (account) =>
                                account.accountName.includes(searchTerm) ||
                                account.accountAcronym.includes(searchTerm),
                            )
                            .map((account) => {
                              return (
                                <Tr key={account.id}>
                                  ...
                                  <Td textAlign="right">
                                    <ButtonGroup variant="outline" spacing="6">
                                      <Button
                                        ...
                                        onClick={() =>
                                          router.push(
                                            `/dashboard/accounts/edit/${account.id}`,
                                          )
                                        }
                                      >
                                        edit
