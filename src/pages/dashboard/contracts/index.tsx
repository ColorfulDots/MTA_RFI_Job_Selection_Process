...
export const DashboardContracts = (props: any) => {
  ...
  const { data: contractsData } = useCollection<any>('contracts', {
    orderBy: ['createdAt', 'desc'],
  });

  const { deleteDocument } = useDocument<any>(
    deleteId ? `contracts/${deleteId}` : null,
  );
  ...
  const handleDelete = async () => {
    ...
    toast({
      ...
      description: "You've successfully deleted an contract!",
      status: 'success',
      duration: 5000,
      isClosable: true,
    });
  ...
  return (
    <>
      {contractsData && (
        <LayoutDashboard title="Contracts" bg={props.bg} color={props.color}>
                ...
                <Box as="section" py="16">
                  ...
                  >
                    <Box>
                      ...
                      <TableActions
                        csvData={contractsData}
                        csvHeaders={null}
                        type="Contract"
                        count={contractsData?.length}
                        newLink={'/dashboard/contracts/new'}
                          ...
                          {contractsData
                            ?.filter(
                              (contract) =>
                                ...
                                contract.contractAcronym.includes(searchTerm),
                            )
                            .map((contract) => {
                              return (
                                <Tr key={contract.id}>
