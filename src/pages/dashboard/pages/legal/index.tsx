...
export const DashboardPagesLegal = (props: any) => {
  ...
  const { data: legalsData } = useCollection<any>('legals', {
    listen: false,
    orderBy: ['title', 'asc'],
  });

  const { deleteDocument } = useDocument<any>(
    deleteId ? `legals/${deleteId}` : null,
  );
  ...
  const handleDelete = async () => {
    ...
    toast({
      title: 'Legal Post Deleted!',
      description: "You've successfully deleted a legal post!",
      status: 'success',
      duration: 5000,
      isClosable: true,
    });
  ...
  return (
    <>
      {legalsData && (
        <LayoutDashboard title="Legal" bg={props.bg} color={props.color}>
                  ...
                  >
                    <Box>
                      <Heading size="lg" mb="6">
                        Legal Posts ({legalsData?.length})
                      </Heading>
                      <TableActions
                        legalsData={legalsData}
                        type="Legal"
                        count={legalsData?.length}
                        newLink={'/dashboard/pages/legal/new'}
                      ...
                      <Table variant="simple" my={4}>
                        ...
                        <Tbody>
                          {legalsData
                            ?.filter((legal) =>
                              legal.title.includes(searchTerm),
                            )
                            .map((legal) => {
                              return (
                                <Tr key={legal.id}>
                                  <Td>
                                    <NextLink href={`/legal/${legal.slug}`}>
                                      {legal.title}
                                    </NextLink>
                                  </Td>
                                  <Td>{legal.visibility}</Td>
                                  <Td textAlign="right">
                                    <ButtonGroup variant="outline" spacing="6">
                                      <Button
                                        ...
                                        onClick={() =>
                                          router.push(
                                            `/dashboard/pages/legal/edit/${legal.id}`,
                                          )
                                        }
                                      >
                                        edit
...
export default DashboardPagesLegal;
